...
class SMMAgentAutodim(DDPGAgent):
    ...
    def delayed_init(self, state_preprocessor):
        # self.obs_dim is now the real obs_dim (or repr_dim) + z_dim
        self.state_preprocessor = state_preprocessor
        self.smm = SMM(self.state_preprocessor.output_dim,
                       self.z_dim,
        ...
        self.ft_returns = np.zeros(self.z_dim, dtype=np.float32)
        self.ft_not_finished = [True for z in range(self.z_dim)]
    ...
    def update(self, replay_iter, step):
        ...
        obs_original = torch.clone(obs)
        obs = self.state_preprocessor(obs)
        with torch.no_grad():
            next_obs = self.aug_and_encode(next_obs)
            next_obs_original = torch.clone(next_obs)
            next_obs = self.state_preprocessor(next_obs)
        obs_z = torch.cat([obs, z], dim=1)  # do not learn encoder in the VAE
        next_obs_z = torch.cat([next_obs, z], dim=1)
        obs_original_z = torch.cat([obs_original, z], dim=1)  # do not learn encoder in the VAE
        next_obs_original_z = torch.cat([next_obs_original, z], dim=1)
        ...
        # update critic
        metrics.update(
            self.update_critic(obs_original_z.detach(), action, reward, discount,
                               next_obs_original_z.detach(), step))

        # update actor
        metrics.update(self.update_actor(obs_original_z.detach(), step))
